...
func Rebuild(
	...
	src.IterateStacks(func(_ model.LocationRefName, self int64, stack []model.LocationRefName) {
		...
		if len(stack) > 0 && stack[0] == 0 {
			// The root-most ref in every stack from a tree that has gone
			// through UnmarshalTree is a synthetic artifact of the tree
			// marshal format, not a real reference; drop it.
			stack = stack[1:]
		}
		expanded = expanded[:0]
